...
try:
    def get_numbers_ticket(min: int, max: int, quantity: int) -> list:
        if 1 <= min < max <= 1000 and quantity <= int(max - min):
            list_all_numbers = list(range(min, max + 1))
            list_winning_numbers = random.sample(list_all_numbers, quantity)
            result = sorted(list_winning_numbers)
        else:
            print('Введіні числа не відповідають умові')
            result = []
        return result
    lottery_numbers = get_numbers_ticket('1', 4000, 10)
    print("Ваші лотерейні числа:", lottery_numbers)
except UnboundLocalError:
    print(f'Кількість вказаних номерів на лотерейному квитку більше ніж кількість чисел у діапазоні max-min.')
    print([])
except NameError:
    print(f'Введіть, будь ласка, число, а не літеру.')
    print([])
except TypeError:
    print(f'Не корректний тип введених даних.')
    print([])
